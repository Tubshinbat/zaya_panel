...
const News = (props) => {
  ...
  const init = () => {
    ...
    props.loadOnlineCourses(`select=${select}`);
  };

  const addClick = () => {
    props.history.push("/online-video/add");
  };
  ...
  const deleteClick = () => {
    ...
    props.deleteMultOnlineCourse(ids);
    setDeleteModel(false);
  };
...
const mapStateToProps = (state) => {
  return {
    courses: state.onlineCourseReducer.onlineCourses,
    pageLast: state.onlineCourseReducer.paginationLast,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    loadOnlineCourses: (query) => dispatch(actions.loadOnlineCourses(query)),
    loadPagination: (pageLast) => dispatch(actions.loadPagination(pageLast)),
    deleteMultOnlineCourse: (ids) =>
      dispatch(actions.deleteMultOnlineCourse(ids)),
  };
};
