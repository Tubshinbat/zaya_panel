...
const Add = (props) => {
  ...
  useEffect(() => {
    if (props.course) {
      setForm(() => ({
        ...props.course,
        group: props.course.group && props.course.group._id,
        createUser: props.course.group && props.course.createUser._id,
      }));

      if (props.course.pictures) {
        setPhotos(props.course.pictures);
        setForm((bf) => ({ ...bf, pictures: props.course.pictures }));
        setErrors((bf) => ({ ...bf, pictures: true }));
      }

      if (props.course.video) {
        setVideo(props.course.video);
        setForm((bf) => ({ ...bf, video: props.course.video }));
      }
  ...
  const init = () => {
    ...
    props.getOnlineCourse(props.match.params.id);
    props.clear();
  };
  ...
  const convertFromdata = () => {
    ...
    Object.keys(formData).map((index) => {
      if (index === "pictures" || index === "video" || index === "teachers") {
  ...
  const addClick = () => {
    ...
    allCheck() === true
      ? props.updateOnlineCourse(props.match.params.id, sendData)
      : toastControl(
          "error",
  ...
  return (
    <Section>
      <MetaTags>
        <title> Онлайн сургалтын хичээл шинчлэх | WEBR Control Panel</title>
        <meta
          name="description"
          content="Онлайн сургалтын хичээл шинчлэх | WeBR control panel"
        />
        <meta
          property="og:title"
          content="Онлайн сургалтын хичээл шинчлэх | web control panel"
        />
      </MetaTags>

      <PageTitle name={`Онлайн сургалтын хичээл шинчлэх `} />
      <div className="row">
              ...
              <div className={css.CategoryBox}>
                <div className="card-body box-profile">
                  ...
                  <div className={css.Thumb}>
                    {video && (
                      <EditVideo file={`${video}`} index={`${video}`} />
                    )}
                  </div>
                  {errors.video && (
...
const mapStateToProps = (state) => {
  return {
    ...
    course: state.onlineCourseReducer.onlineCourse,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    updateOnlineCourse: (id, data) =>
      dispatch(actions.updateOnlineCourse(id, data)),
    loadOnlineGroups: (query) => dispatch(loadOnlineGroups(query)),
    loadEmployees: (query) => dispatch(loadEmployees(query)),
    getOnlineCourse: (id) => dispatch(actions.getOnlineCourse(id)),
