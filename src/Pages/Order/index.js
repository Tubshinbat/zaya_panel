...
const Serives = (props) => {
  ...
  const [select, setSelect] = useState(
    "status name slug  pictures type createAt"
  );
  ...
  useEffect(() => {
    props.loadOrders(
      `select=${select}&sort=${sort}&ordernumber=${search.ordernumber}&payis=${search.payis}&orderType=${search.orderType}&user=${search.user}&page=${activePage}`
    );
  }, [search]);
  ...
  return (
    <Section>
      <MetaTags>
        <title> Бараа захиалга | WEBR Control Panel</title>
        <meta
          name="description"
          content="Бараа захиалга | WeBR control panel"
        />
        <meta
          property="og:title"
          content="Бараа захиалга | web control panel"
        />
      </MetaTags>
      <PageTitle name={`Бараа захиалга`} />
        ...
        <div className={css.PanelControl}>
          <div className="col-md-4">
            <div className={css.PanelTabelHeader}>
              ...
              >
                <i className="fas fa-plus-circle"></i> Бараа захиалга нэмэх{" "}
              </button>
              <button
                name="refresh"
                onClick={() => init()}
                ...
                {props.orders &&
                  props.orders.map((el) => (
                    <tr key={el._id}>
                      ...
                      <td>
                        <a
                          href={`${myBase.siteUrl}product/${
                            el.product && el.product.slug
                          }`}
                          target="_blank"
                        >
...
const mapStateToProps = (state) => {
  return {
    ...
    pageLast: state.orderReducer.paginationLast,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    loadOrders: (query) => dispatch(actions.loadOrders(query)),
