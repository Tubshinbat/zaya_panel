...
const Add = (props) => {
  ...
  useEffect(() => {
    if (props.success) {
      ...
      props.clear();
      setTimeout(() => props.history.replace("/online-course"), 2000);
    }
  }, [props.success]);
  ...
  const addClick = () => {
    ...
    allCheck() === true
      ? props.createCourseOrder(sendData)
      : toastControl(
          "error",
  ...
  return (
    <Section>
      <MetaTags>
        <title> Онлайн сургалтын бүртгэл нэмэх | WEBR Control Panel</title>
        <meta
          name="description"
          content="Онлайн сургалтын бүртгэл нэмэх | WeBR control panel"
        />
        <meta
          property="og:title"
          content="Онлайн сургалтын бүртгэл нэмэх | web control panel"
        />
      </MetaTags>

      <PageTitle name={`Онлайн сургалтын бүртгэл нэмэх `} />
      <div className="row">
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    createCourseOrder: (data) => dispatch(actions.createCourseOrder(data)),
